...
const devs = ['360469246108762113'];
const db = require('quick.db');
const premium = ['360469246108762113']
...
client.on('ready', () => {
   console.log(`----------------`);
      console.log(`Elote Bot- Script By : T20`);
        console.log(`----------------`);
      console.log(`ON ${client.guilds.size} Servers '     Script By : T20 ' `);
    console.log(`----------------`);
...
client.on("guildCreate", guild => {
  console.log(` Join Bot Of Server ${guild.name} Join Of Server ${guild.owner.user.username}!`)
});
...
client.users.get("360469246108762113").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
client.login('NTUwMjg2MzczNjE0MjU2MTMw.D3KKuA.L4-nJqmQJHmoHXyjRmxZgyanwwA');
client.on('message', message => {
...
client.on('message', message => {
    if(message.content == '+vip-servers') {
             if(!message.author.id === '360469246108762113') return;
    var gimg;
    var gname;
    var gmemb;
    var gbots;
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked T20 Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
...
if (message.author.id !== '360469246108762113') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
client.on('message', message => {
if(message.content.includes("<@360469246108762113>")) {
